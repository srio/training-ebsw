...
def wiggler_preprocessor(ener_gev=6.0,e_min=5000.0,e_max=5005.0,file_field="",dir_file_field="./",
                        plot_trajectories=0,shift_x_flag=0,shift_x_value=0.0,shift_betax_flag=0,shift_betax_value=0.0):

    (traj, pars) = srfunc.wiggler_trajectory(b_from=1,
                                             inData=dir_file_field+file_field,
                                             nPer=1,
                                             nTrajPoints=501,
                                             ener_gev=ener_gev,
                                             per=None,
    ...
    if plot_trajectories > 0:
        ...
        if plot_trajectories == 2:
            plt.show()

        fig.savefig('GRAPHICS/sw_'+file_field+'.png')
        plt.close(fig)
...
def wiggler_source(ener_gev=6.0,e_min=5000.0,e_max=5005.0,iwrite=0,emittance=0):
...
if __name__ == "__main__":
    ...
    # initialize
    #
    plot_trajectories = write_html_report  # 0=No, 1=png file, 2=png+window
